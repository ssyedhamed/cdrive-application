...
var letters = /^[A-Za-z0-9_ ]+$/;
	...
	if(query==""){
		$('.query-box').css({"display":"none"})
		
 $('.search-input').css({ "border-radius": "20px" });
		return;
	}
	if(query.match(letters)){
	const apiEndpoint=`http://localhost:4000/search/${query}`;
	...
	data.forEach(contact=>{
		div+=`<a class='list-group-item list-group-action' href='/user/${contact.id}/show_contact/0'>${contact.name}</a>`
	})
	
	div+=`</div>`
 $('.search-input').css({ "border-radius": "20px 20px 0px 0px" });
	$('.query-box').html(div);
	})
$('.query-box').css({"display":"block"})
	}else{
$('.query-box').css({"display":"none"})
